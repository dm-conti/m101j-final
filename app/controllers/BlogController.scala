...
trait BlogControllerApi extends Controller {
  ...
  def processLike = Action { implicit request =>
    likeForm.bindFromRequest.fold(
      withErrors => BadRequest(views.html.postNotFound()),
      {
        case (permalink, ordinal) => blogPostDAO.findByPermalink(permalink) match { // look up the post in question
          case Some(post) =>
            blogPostDAO.likePost(permalink, ordinal)
            Redirect(routes.BlogController.postDetail(permalink))
          case _ => Ok(views.html.postNotFound()) //  if post not found, redirect to post not found error
        }
      })
  }
